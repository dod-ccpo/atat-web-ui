...
describe("Testing ATATSummaryCard Component", () => {
  ...
  const mockcard: ATATSummaryCardItem = {
    type: "TASK ORDER",
  ...
  it("PORTFOLIO update success", async () => {
    ...
    expect(wrapper.vm.truncateText(portfolioCard.title, 10)).toBe(
      "Mock Port..."
    );
  });
  ...
  it("APPLICATION update success", async () => {
    ...
    expect(wrapper.vm.truncateText(applicationCard.title, 10)).toBe(
      "Mock Appl..."
    );
  });
