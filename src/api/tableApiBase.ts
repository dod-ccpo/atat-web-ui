...
export  class TableApiBase<TableDTO> extends baseApi  {
     ...
     private async post<TData>(data?: TData, config?:AxiosRequestConfig):Promise<AxiosResponse>{
           return this.instance.post(this.endPoint, data, config);
     }

     private async get(sys_id: string, config?:AxiosRequestConfig):Promise<AxiosResponse>{
     ...
     private async delete(sys_id:string, config?:AxiosRequestConfig): Promise<AxiosResponse>{
         return this.instance.delete(this.urlWithSysId(sys_id),config);
     }

     private async patch<TData>(sys_id: string, data?:TData, config?:AxiosRequestConfig):Promise<AxiosResponse>{
        ...
        async retrieve(sys_id: string): Promise<TableDTO> {
            try{
              ...
              if (response.status === 200) {
                ...
                throw new Error(`unable to retrieve ${this.tableName} with sys_id: ${sys_id}`);
              }
            } catch (error) {
