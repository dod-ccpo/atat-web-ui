...
export class UserApi extends ApiBase {
  ...
  public async search(searchStr: string, searchCol?: string): Promise<UserDTO[]> {
    ...
    try {
      let searchQuery = "";
      if (searchCol) {
        searchQuery = searchCol + "STARTSWITH" + searchStr
      } else {
        // eslint-disable-next-line max-len
        searchQuery = `^first_nameSTARTSWITH${searchStr}^ORlast_nameSTARTSWITH${searchStr}^ORemailSTARTSWITH${searchStr}^emailISNOTEMPTY`;
