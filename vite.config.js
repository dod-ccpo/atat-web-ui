...
export default defineConfig(({ command, mode}) => {
  ...
  const BASE_API_URL = env.BASE_API_URL.endsWith("/") 
      ? env.BASE_API_URL + "api" : env.BASE_API_URL + "/api",
    SNOWUSER =  mode === 'development' ? env.SNOWUSER:'',
    SNOWPASS = mode === 'development' ? env.SNOWPASS:'',
    SNOW_USER_SYSID = mode === 'development' 
      ? env.userId : "e0c4c728875ed510ec3b777acebb356f" // pragma: allowlist secret
  ...
  return {
    ...
    css: {
      extract: {// false // puts it into app-js
