...
describe("Test suite: Background- Current Environment: Functional Testing1", () => {
  const pt = "TC-Step-04-Background:Current Environment-Functional-" + randomAlphaNumeric(5);
  ...
  const storageOptions = ['Object' //Block, Object, File, Archive
  ]
  ...
  // Summary page:
  const deployedRegionCheckboxesList = deployedRegionCheckboxes.join(' ,')
  ...
  it("TC1: Current Environment: Functional Testcase- Screen Validation Pages1-5", () => {
    ...
    cy.verifyRadioGroupLabels(background.existingEnvNoRadioGroup, 
      bgCEData.CEPage1.section1Radioboxes);
    environmentsystemDiagramsMigrationOptions("Yes", background.existYesRadioOption, 
    ...
    environmentsystemDiagramsMigrationOptions("Yes", background.systemDocsYesRadioBtn, 
      background.systemDocsNoRadioBtn);

    cy.findElement(background.fileUploadSection).should("exist").and("be.visible");
    cy.findElement(background.dragandDropText).should("contain.text", 
      bgCEData.CEPage2.dragandDropText);
    cy.findElement(background.browsetoUploadText).should("contain.text", 
      bgCEData.CEPage2.browseToUploadText);
    cy.findElement(background.supportFileText).should("contain.text", 
    ...
    cy.verifyRadioGroupLabels(background.envLocationRadioGroup, 
  ...
  it("TC2: Current Environment: Functional Testcase- Screen Validation pages 6-8", () => {
    ...
    // Page#2:  Do you have system diagrams, data architecture diagrams, charts etc..? 
    cy.clickContinueButton(background.existYesRadioOption, bgCEData.CEPage2.pageHeader2)
    environmentsystemDiagramsMigrationOptions("No", background.systemDocsYesRadioBtn,
      background.systemDocsNoRadioBtn);

    // Page#3:  Have you completed a migration assessment, .... and tools needed?  
    cy.clickContinueButton(background.systemDocsYesRadioBtn, bgCEData.CEPage3.pageHeader3);
    environmentsystemDiagramsMigrationOptions("No", background.existYesRadioOption,
      background.existNoRadioOption);
    ...
    cy.verifyRadioGroupLabels(background.classificationRadioGroup, 
    ...
    cy.verifyRadioGroupLabels(background.currentUsageRadioGroup,
    ...
    cy.findElement(background.blockStorageOption + background.storageSubtitle)
    ...
    cy.findElement(background.objectTypeStorageOption + background.storageSubtitle)
    ...
    cy.findElement(background.fileStorageOption + background.storageSubtitle)
    ...
    cy.findElement(background.archiveStorageOption + background.storageSubtitle)
    ...
    cy.waitUntil(function () {
      return cy.findElement(background.storageSizeField + background.gigabyteOption)
        .should("exist");
    })
    cy.findElement(background.storageSizeField + background.gigabyteOption)
      .should("contain.text", " Gigabyte (GB) ");
    cy.findElement(background.storageSizeField + background.terabyteOption)
      .should("contain.text", " Terabyte (TB) ");
    cy.findElement(background.storageSizeField + background.petayteOption)
    ...
    cy.textExists(background.performanceTiertitle, bgCEData.CEPage6.performanceTierLable);
    cy.verifyRadioGroupLabels(background.performanceTierRadioGroup, 
      bgCEData.CEPage6.performanceRadioboxes);

    cy.radioBtn(background.generalPurposeRadiobox, "GENERAL").click({
      force: true
    });
    cy.textExists(background.instancesTextboxLable, bgCEData.CEPage6.instancesTextboxLable); 
    cy.findElement(background.instancesTextbox).clear().type(noOfInstances);

    cy.textExists(background.dataegressTextboxLable, bgCEData.CEPage6.monthlyDataTextboxLable); 
    ...
    cy.verifyRadioGroupLabels(background.currentPaymentRadioGroup,
    ...
    cy.verifyTableData(background.summaryCETableHeader, 
      background.summaryCETableData, "Location", deployedRegionCheckboxesList)
    cy.verifyTableData(background.summaryCETableHeader,
      background.summaryCETableData, "Classification", "UNCLASSIFIEDIL5");
    cy.verifyTableData(background.summaryCETableHeader,
      background.summaryCETableData, "Quantity", noOfInstances);
    cy.verifyTableData(background.summaryCETableHeader,
      background.summaryCETableData, "vCPU", noOfVCPUs);
    cy.verifyTableData(background.summaryCETableHeader,
      background.summaryCETableData, "Memory", expctedMemory);
    cy.verifyTableData(background.summaryCETableHeader, 
      background.summaryCETableData, "Storage", expctedStorage);
    cy.verifyTableData(background.summaryCETableHeader, 
      background.summaryCETableData, "Performance", "GENERALPURPOSE");

    cy.clickContinueButton(background.currentEnvHistoryTable, 
    ...
    cy.textExists(background.currentEnvironmentHeaderText, 
    ...
    cy.findElement(background.currentEnvironmentDescription)
      .contains("1 instance (Unclassified/IL5)");
    cy.textExists(background.currentEnvironmentCompleteBtn, 
  ...
  it("TC4: Current Environment: Functional Testcase- Error message validation", () => {
    ...
    //Section#1
    cy.btnClick(common.continueBtn, " Continue ");
    cy.checkErrorMessage(background.classandImpactErrorMessage, 
      bgCEData.CEPage6.classificationImpactMessageText);

    // //Section#2
    cy.checkErrorMessage(background.currentUsageErrorMessage, 
    ...
    cy.btnClick(common.continueBtn, " Continue ");
    cy.checkErrorMessage(background.spikesCausesErrorMessage,
    ...
    cy.checkErrorMessage(
      background.performanceTierErrorMessage, bgCEData.CEPage6.performanceTierMessageText);
    cy.checkErrorMessage(
    ...
    cy.findElement(
      background.additionalInfoTextbox).click().type(additionalInformationText); //#Section5
