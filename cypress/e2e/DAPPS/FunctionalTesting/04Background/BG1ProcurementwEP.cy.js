...
describe(
  "Test suite: Step04-Previous or Current Contract with EvaluationPlan",
  { testIsolation: false },
  () => {
    ...
    const descriptionDetails = [
      `${rowCount} previous contract: ${validContractNo}`,
    ];
    //updated description after edit
    const updatedDescriptionDetails = [
      `${rowCount} previous contract: ${edittedContractNo}`,
    ];
    ...
    it("TC1:Message Validations: Current Contract", () => {
      ...
      cy.radioBtn(background.ccNoRadioOption, "NO").focus().focus();
      cy.clickSomethingElse(background.radioOptionError).then(() => {
        cy.checkErrorMessage(
          background.radioOptionError,
          "Please select an option"
        );
      });
      cy.findElement("#developerToggleButton").click({
        force: true,
      });
    ...
    it("TC2: Validations: Let’s gather some details about your previous or current contract", () => {
      cy.log(
        " TestReport: Let’s gather some details about your previous or current contract"
      );
      cy.verifyPageHeader(
        "Let’s gather some details about your previous or current contract"
      );
      ...
      cy.clickSomethingElse(background.contractNoTxtBox).then(() => {
        cy.findElement(background.contractNoTxtError).should("not.exist");
      });
      cy.findElement(background.contractNoTxtBox).clear();

      cy.findElement(background.contractNoTxtBox)
        .type(contractNo)
        .blur({
          force: true,
        })
      ...
      cy.findElement(background.expirationDatePickerInputbox)
        .should("be.visible")
        .clear()
        .click()
        .blur({
    ...
    it("TC4:Procurement History Details-view/Edit", () => {
      ...
      cy.verifyEnteredInputTxt(
        background.expirationDatePickerInputbox,
