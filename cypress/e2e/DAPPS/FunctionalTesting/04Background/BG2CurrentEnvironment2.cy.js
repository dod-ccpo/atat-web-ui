...
describe("Test suite: Background- Current Environment: Functional Testing", () => {
  ...
  function section3InstanceConfigurations() {
    ...
    cy.waitUntil(function () {
      return cy.findElement(
        background.storageSizeField + background.gigabyteOption).should("exist");
    })
  ...
  function section7verifyTableData(impactLevelCheckboxesList) {
    ...
    cy.verifyTableData(background.summaryCETableHeader, 
      background.summaryCETableData, "Classification", impactLevelCheckboxesList);
    cy.verifyTableData(background.summaryCETableHeader,
      background.summaryCETableData, "Quantity", instances);
    cy.verifyTableData(background.summaryCETableHeader, 
      background.summaryCETableData, "vCPU", noOfVCPU);
    cy.verifyTableData(background.summaryCETableHeader, 
      background.summaryCETableData, "Memory", expctedMemory);
    cy.verifyTableData(background.summaryCETableHeader, 
      background.summaryCETableData, "Storage", storageExpected);
    cy.verifyTableData(background.summaryCETableHeader, 
      background.summaryCETableData, "Performance", performanceTierOptions);
  ...
  it("TC1: Current Environment: Functional Testcase- Hybrid-Cloud Environment", () => {
    ...
    cy.verifyRadioGroupLabels(background.instanceRadioGroup, 
    ...
    cy.verifyTableData(background.summaryCETableHeader, 
      background.summaryCETableData, "Location", "CLOUD" + deployedRegionCheckboxesList);
    ...
    cy.findElement(background.currentEnvironmentDescription)
      .contains("1 cloud instance (Unclassified/IL4)");
  ...
  it("TC2: Current Environment: Functional Testcase- Hybrid-OnPremise Environment", () => {
    ...
    cy.textExists(background.introPText, bgCEData.CEPage7.pageText7);
    cy.verifyTableData(background.summaryCETableHeader, 
      background.summaryCETableData, "Location", "ON-PREMISEMISSINGINFO");
    ...
    cy.textExists(background.currentEnvironmentDescription, "Hybrid environment:");
    cy.findElement(background.currentEnvironmentDescription)
      .contains("1 on-premise instance (Secret/IL6)");
    // cy.textExists(background.currentEnvironmentCompleteBtn, 
    //   bgCEData.BackgroundSummary.currentEnvironmentViewButton);
    cy.textExists(background.currentEnvironmentReviewBtn, 
