...
describe(
  "Test suite: Functional Testing - 03 Contract Details> ClassificationRequirements",
  { testIsolation: false },
  () => {
    const pt = "TC-Step-3-ContractDetails-ClassLevel-" + randomAlphaNumeric(5);
    const scope = "Project Scope-" + randomString(5);

    const classInput = randomNumber(2);
    const projectFSText =
      "Test Projected FileStream - " + randomAlphaNumeric(3);
    const anticipatedText =
      "Test Anticipated Usage Textbox - " + randomAlphaNumeric(17);
    ...
    beforeEach(() => {
      ...
      cy.clickSideStepper(
        common.subStepClassReqsLink,
        " Classification Requirements "
      );
    });

    it("TC1: Functional Testing: Screen Validation: Classification Requirements", () => {
      cy.log(" TestReport: Step3-validations "); // select all check boxes in page#1
      cy.log(
        " TestReport: Verify header and text messages and select all check boxes in Page#1 "
      );
      cy.verifyPageHeader(CDData.classLevelPage1.pageHeaderCL1);
      cy.verifyTextMatches(
        contractDetails.introPText,
        CDData.classLevelPage1.expectedintroText
      );
      cy.textExists(
        contractDetails.selectMess,
        CDData.classLevelPage1.selectALLMsgCLPage
      );
      cy.verifyCheckBoxLabels(
        contractDetails.classCheckBoxes,
        CDData.classLevelPage1.expectedLabels
      );
      cy.findElement(contractDetails.classCheckBoxes).should("not.be.checked");
      cy.findElement(contractDetails.classCheckBoxes).check({
        force: true,
      });
      cy.findElement(contractDetails.classCheckBoxes).should("be.checked");
      cy.verifyTextMatches(
        contractDetails.alertMessage,
        CDData.classLevelPage1.alertMessage
      );
      ...
      cy.log(" TestReport: page#2 validations "); // select all check boxes in page#2
      cy.verifyPageHeader(CDData.classLevelPage2.pageHeaderCL2);
      cy.verifyTextMatches(
        contractDetails.introPText,
        CDData.classLevelPage2.cspMessage
      );
      cy.textExists(
        contractDetails.messageSecret,
        CDData.classLevelPage2.scSection
      );
      cy.textExists(
        contractDetails.messageNoteSecret,
        CDData.classLevelPage2.selectALLSecretMsg
      );
      cy.verifyCheckBoxLabels(
        contractDetails.secretCheckbox,
        securityRequirements.secret.checkboxes
      );
      cy.get(contractDetails.secretCheckbox)
        ...
        .should("be.checked");
      cy.textExists(
        contractDetails.messageTopSecret,
        CDData.classLevelPage2.tsSection
      );
      cy.textExists(
        contractDetails.messageNoteTopSecret,
        CDData.classLevelPage2.selectALLTopSecretMsg
      );
      cy.verifyCheckBoxLabels(
        contractDetails.tsCheckbox,
        securityRequirements.topSecret.checkboxes
      );
      cy.get(contractDetails.tsCheckbox)
        .should("not.be.checked")
        .check({
          force: true,
        })
      ...
      cy.log(
        " TestReport: Verify header and text messages and select all check boxes in Page#3 "
      );
      ...
      cy.textExists(
        contractDetails.cdsLabel,
        CDData.classLevelPage3.cdsLabelTxt
      );
      cy.textExists(
        contractDetails.cdsLabel2,
        CDData.classLevelPage3.cdsLabelTxt2
      );
      cy.verifyCheckBoxLabels(
        contractDetails.cdsCheckbox,
        securityRequirements.classCDS.checkboxes
      );

      cy.get(contractDetails.cdsCheckbox)
        .should("not.be.checked")
        .check({
          force: true,
        })
      ...
      cy.textExists(
        contractDetails.projectedFSFieldLabel,
        CDData.classLevelPage3.projectFSFieldLabel
      );
      cy.enterTextInTextField(contractDetails.projectedFSField, projectFSText);

      //verify lables
      cy.textExists(
        contractDetails.anticipatedFieldLabel,
        CDData.classLevelPage3.anticipatedLabel1
      );
      cy.textExists(
        contractDetails.anticipatedFieldLabel2,
        CDData.classLevelPage3.anticipatedLabel2
      );
      ...
      cy.textExists(
        contractDetails.basePeriodLabel1,
        CDData.classLevelPage3.baseLabel1
      );
      cy.radioBtn(contractDetails.entiredDurationYes, "YES")
        .not("[disabled]")
        .click({
          force: true,
        });
      cy.radioBtn(contractDetails.entiredDurationNo, "NO")
        .not("[disabled]")
        .click({
          force: true,
        });
      cy.textExists(
        contractDetails.basePeriodLabel2,
        CDData.classLevelPage3.baseLabel2
      );
    ...
    it.skip("TC2: Functional Testing: Message Validations: Classification Requirements", () => {
      ...
      cy.findElement(contractDetails.level5)
        ...
        .should("be.checked");
      cy.findElement(contractDetails.ts)
        .should("not.be.checked")
        .check({
          force: true,
        })
      ...
      cy.clickContinueButton(
        contractDetails.ts,
        CDData.classLevelPage1.pageHeaderCL2
      );
      ...
      cy.verifyRequiredCheckbox(
        contractDetails.entiredDurationNo,
        contractDetails.basePeriodCheckboxError,
