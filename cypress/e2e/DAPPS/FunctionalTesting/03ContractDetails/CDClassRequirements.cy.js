...
describe("Test suite: Functional Testing - 03 Contract Details> ClassificationRequirements", () => {
  ...
  it("TC1: Functional Testing: Screen Validation: Classification Requirements", () => {
    ...
    cy.textExists(contractDetails.messageNoteTopSecret, 
    ...
    cy.log(" TestReport: Verify header and text messages and select all check boxes in Page#3 ") 
    ...
    cy.clickContinueButton(contractDetails.entiredDurationNo,
  ...
  it.skip("TC2: Functional Testing: Message Validations: Classification Requirements", () => {
    ...
    //Page#3
    cy.verifyRequiredRadioBtn(contractDetails.cdsNoOption, 
      contractDetails.cdsRadioOptionError, "Please select Yes or No.");
    cy.findElement(contractDetails.cdsYesOption).click({
      force: true
    });
    cy.verifyRequiredCheckbox(contractDetails.unclastoSecrCB, 
    ...
    cy.selectCheckBoxes([contractDetails.unclastoSecrCB]).then(() => {
      cy.verifyRequiredInput(contractDetails.cdsUtoSTxtbox, contractDetails.cdsDPTxtbox0Error, 
        "Enter the approximate quantity of data/month.");
    });
    cy.verifyRequiredInput(contractDetails.projectedFSField, 
      contractDetails.projectedFSError, "Enter a projected file stream/type");
    cy.verifyRequiredInput(contractDetails.anticipatedTxtbox, 
      contractDetails.anticipatedTxtboxError, "Provide your statement of objective.");
    cy.verifyRequiredRadioBtn(contractDetails.entiredDurationNo, 
      contractDetails.entiredDurationError, "Please select an option.");
    cy.verifyRequiredCheckbox(contractDetails.entiredDurationNo, 
      contractDetails.basePeriodCheckboxError, "Please select at least one base or option period.");
  });
