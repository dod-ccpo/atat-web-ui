...
describe("Test suite: Functional Testing - 03 Contract Details> Period of Performance", () => {
  ...
  it("TC1: Functional Testing: Screen Validation: Period of Performance", () => {
    ...
    const expOptions = CDData.POPPage1.expectedOptions
    ...
    cy.defaultPoPLengthValue(contractDetails.optionalTextBox, 
    ...
    cy.log(" After Drag and Drop , Base'has 'Option#2' value &'Option#2' textbox has 'Base' value");
    cy.findElement(contractDetails.addOptionLink).should("exist").click();
    ...
    cy.log(" TestReport: Step3-ContractDetails-POP Page#1 Functional Testing complete ")
    ...
    cy.verifyTextMatches(contractDetails.warningTextMessage,
      CDData.POPPage2.expectedWarningMessageNLT);
    ...
    cy.log(" TestReport: Step3-ContractDetails-POP Page#2 Functional Testing complete ")
  ...
  it("TC2: Functional Testing: Message Validations: Period of Performance", () => {
    ...
    cy.findElement(contractDetails.baseInputTxtBox).clear().type("6").then(() => { //Basevalue> 1yr 
      option = " 1 year ";
      showValidationMessage()
    });
    cy.findElement(contractDetails.baseInputTxtBox).clear().type("13") //enter Basevalue> 13months 
    cy.findElement(contractDetails.baseDropdownIcon).click();
    ...
    cy.clickSomethingElse(contractDetails.popStartRadioError)
      .then(() => {
        cy.checkErrorMessage(contractDetails.popStartRadioError, 
          CDData.POPPage2.popStartRadioError);
      });
    cy.radioBtn(contractDetails.popStartDateYesRadioOption, "YES")
      .click({
        force: true
        ...
        cy.clickSomethingElse()
          .then(() => {
            cy.checkErrorMessage(contractDetails.requestDatePickerError, 
              CDData.POPPage2.requestDatePickerError)
          });
      });
