...
import "./04BGProcurementwEP.cy.js";

const dataSets = [epSet1, epSet2, epSet3, epSet4];
...
describe("Test suite: Step04-Background-Current Environment", () => {
  ...
  const {
    ...
    currentPayment,
    currentEnvironmentDescDetails,
    impactLevelCheckboxes
  } = dataSet;
  

  // page#5
  const cloudClassLevelCheckboxes = [
    // Cloud computing
    "UnClassifiedCloud",
    "SecretCloud",
    "TSCloud",
  ];
  const unClassLevelCheckboxes = [
    //cloud Computing
    "levelIL2",
    "levelIL4",
    "levelIL5",
  ];
  const premUnclassLevelCheckboxes = [
    // onPremise
    "unClassifiedPrem",
    "secretPrem",
    "topSecretPrem",
  ];
  const unClassInstancesCheckboxes = [
    // onPremise
    "publicRelease",
    "cUI",
    "nationalSecurity",
  ...
  //section#2 data
  const eventCauseText =
    "Event that causes a surge in usage for testing- " + randomString(3);
  const periodCauseText =
    "period of the year surge in usage for testing- " + randomString(3);
  ...
  const additionalInformationText =
    "Optional Additional Information is--" + randomString(5);
  ...
  // page#5
  const cloudClassificationLevelMap = {
    //cloud computing
  ...
  const unClassificationInstancesMap = {
    //onPremise
  ...
  const deployedRegionCheckboxesList = deployedRegionCheckboxes
    ...
    .toUpperCase()
    .replace(/\s/g, "");
  const impactLevelCheckboxesList = impactLevelCheckboxes
    .join(" ,")
    .toUpperCase();
  ...
  // page#5
  function cloudClassificationandImpactLevels() {
    //cloud computing
    cloudClassLevelCheckboxes.forEach((cloudClassLevelCheckbox) => {
      cy.findElement(
        cloudClassificationLevelMap[cloudClassLevelCheckbox]
      ).click({
        force: true,
      });
    });
  ...
  function premiseClassificationandImpactLevels() {
    //onPremise
    premUnclassLevelCheckboxes.forEach((premUnclassLevelCheckbox) => {
      cy.findElement(
        premUnclassificationLevelMap[premUnclassLevelCheckbox]
      ).click({
        force: true,
      });
    });
    unClassInstancesCheckboxes.forEach((unClassInstancesCheckbox) => {
      cy.findElement(
        unClassificationInstancesMap[unClassInstancesCheckbox]
      ).click({
        force: true,
      });
    });
  ...
  //Page#6
  function cloudRegionsDeployed() {
    //cloud computing
    deployedRegionCheckboxes.forEach((deployedRegionCheckbox) => {
      cy.findElement(
        background.regionDeployedCheckboxes +
          deployedRegionsMap[deployedRegionCheckbox]
      ).click({
        force: true,
      });
    });
  ...
  function cloudandPremiseDataClassification() {
    //cloud &onPremise
  ...
  function currentUsageandUsersSection2() {
    //#Section2: Current usage and users
    ...
    deployedRegionCheckboxes.forEach((deployedRegionCheckbox) => {
      cy.findElement(
        background.regionUsersCheckboxes +
          deployedRegionsMap[deployedRegionCheckbox]
      ).click({
        force: true,
      });
      cy.findElement(regionUsersTextMap[deployedRegionCheckbox]).type(
        noOfUsers
      );
    });
  }

  function instanceConfigurationsSection3() {
    //#Section3 : Instance configurations
  ...
  function verifyTableData(columnHeader, expectedValue) {
    cy.get(background.summaryCETableHeader).each(($el, index) => {
      const text = $el.text();
      if (text.includes(columnHeader)) {
        cy.get(background.summaryCETableData)
          .eq(index)
          .then(function (value) {
  ...
  function getCurrentContract(fairOpp) {
    if (fairOpp === "none") {
      return currentContract;
    }
  }

  function procurementHistoryDescriptionDetails() {
    let targetDescriptionDetails;     
    const currentContract = getCurrentContract("none");
    cy.log(currentContract);
  
    if (currentContract === "Yes") {
      targetDescriptionDetails = descriptionDetails;
    } else {
      targetDescriptionDetails = [noContract];
    }
  
    cy.verifyListMatches(
      background.procurementHistoryDescription,
      targetDescriptionDetails
    );
  }
  ...
  it("TC1: Current Environment: E2E Happy path", () => {
    ...
    cy.findElement(background.uploadFileSysDiagram).selectFile(
      "cypress/fixtures/files/testFileSysDiagram.docx",
      {
        force: true,
      }
    );
    ...
    // Page#3: Have you completed a migration assessment, analysis,
    // or process to identify the cloud services and tools needed?
    cy.clickContinueButton(
    ...
    cy.findElement(background.uploadFileSysDiagram).selectFile(
      "cypress/fixtures/files/testMigration.xlsx",
      {
        force: true,
      }
    );
    ...
    cy.findElement(currentEnvironmentMap[currentEnvironment]).click({
      //Cloud computing //On premise //Hybrid environment
      force: true,
    });
    cy.clickContinueButton(
    ...
    if (currentEnvironment === "Cloud") {
      ...
      cy.clickContinueButton(
        background.unClassPremCheckbox,
        bgCEData.CEPage6.pageHeader6
      ); //page#6
      cloudandPremiseDataClassification(); //section#1
      currentUsageandUsersSection2(); //section#2
      ...
      if (hybridRadioboxes === "Cloud") {
        ...
        cy.findElement(background.additionalInfoTextbox)
          .click()
          .type(additionalInformationText); //#Section5
      } else if (hybridRadioboxes === "Onpremise") {
        //onPremise
        cloudandPremiseDataClassification(); //section#1
        currentUsageandUsersSection2(); //section#2
        instanceConfigurationsSection3(); //section#3
    ...
    cy.findElement(background.summaryEnvironmentType).contains(
    ...
    cy.clickContinueButton(
      background.summaryEnvironmentType,
