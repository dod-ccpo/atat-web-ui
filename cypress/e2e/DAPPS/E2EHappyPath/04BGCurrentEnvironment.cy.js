...
describe("Test suite: Step04-Background-Current Environment", () => {
  ...
  // page#4
  const currentEnvironment = "Cloud"; // "Cloud", "Onpremise" , "Hybrid"
  // when CurrentEnvironment is "Hybrid" choose below:
  const hybridRadioboxes = "Cloud"; // "Cloud", ,"Onpremise"  

  // page#5
  const cloudClassLevelCheckboxes = [ // Cloud computing
    'UnClassifiedCloud',
    'SecretCloud',
    'TSCloud'
  ]
  const unClassLevelCheckboxes = [ //cloud Computing
    'levelIL2',
    'levelIL4',
    'levelIL5'
  ]
  const premUnclassLevelCheckboxes = [ // onPremise
    'unClassifiedPrem',
    'secretPrem',
    'topSecretPrem'
  ]
  const unClassInstancesCheckboxes = [ // onPremise
    'publicRelease',
    'cUI',
    'nationalSecurity'
  ]
  ...
  const impactLevelCheckboxes = [ // accepts only one option
    // 'UnclassifiedIL2',
    //'UnclassifiedIL4',
    //'UnclassifiedIL5',
    'SecretIL6',
  ...
  const currentUsage = "evenlyDistributed" //evenlyDistributed, irregularUsage
  const spikesUsage = "periodBased" // eventBased, periodBased
  ...
  const currentPayment = 'reserved' //reserved, payAsyouGo
  ...
  function currentUsageandUsersSection2() { //#Section2: Current usage and users
    ...
    deployedRegionCheckboxes.forEach(deployedRegionCheckbox => {
      cy.findElement(background.regionUsersCheckboxes + deployedRegionsMap[deployedRegionCheckbox])
        .click({
          force: true
        });
  ...
  it("TC1: Current Environment: E2E Happy path", () => {       
    ...
    cy.findElement(background.uploadFileSysDiagram)
      .selectFile("cypress/fixtures/files/testFileSysDiagram.docx", {
        force: true
      });
    cy.waitUntil(function () {
    ...
    cy.findElement(background.uploadFileSysDiagram)
      .selectFile("cypress/fixtures/files/testMigration.xlsx", {
        force: true
      });
    cy.waitUntil(function () {
      return cy.findElement(background.removeFile1).should("exist");
    ...
    cy.findElement(currentEnvironmentMap[currentEnvironment])
      .click({ //Cloud computing //On premise //Hybrid environment
        force: true
      });
    ...
    if (currentEnvironment === "Cloud") { //cloud computing
      ...
      cy.clickContinueButton(
        background.unClassCloudCheckbox, bgCEData.CEPage6.pageHeader6); //page#6
      ...
      cy.findElement(background.additionalInfoTextbox)
        .click().type(additionalInformationText); //#Section5
