...
describe("Test suite:Step04-Procurement History with J&A", () => {
  ...
  const previousContract = "Yes"; // Yes or No
  ...
  function popStartEndDate(dayOfMonth){
    ...
    cy.waitUntil(function () {
      return cy.findElement(background.startDateTextField).should("have.value",startDayFormatted);
    });
    cy.selectDatefromDatePicker(
  ...
  function gatherContractDetails(
    contractNo,taskOrderNo,dayOfMonth,cName,expectedProcurementHistoryData){
  ...
  it("TC:1 Procurement History", () => {     
    ...
    if (fairOpp === "YES_FAR_16_505_B_2_I_C") {    
      ...
      cy.clickAndWaitForVisible(
        common.substepProcurementText, background.addInstanceNoDataBtn);        
      ...
      fairOpp === "YES_FAR_16_505_B_2_I_A" 
        || fairOpp === "YES_FAR_16_505_B_2_I_B") {                
      ...
      if (previousContract === "Yes") {
        ...
        gatherContractDetails(
          contractNo,taskOrderNo,dayOfMonth,cName,
