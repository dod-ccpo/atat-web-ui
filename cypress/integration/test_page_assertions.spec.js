...
describe("TestSuite to Verify the Assertions for all pages", function () {
  ...
  it("Testcase:1:Assertions on ATAT Homepage", function () {
    ...
    const dateTimeStr =
      new Date().toLocaleDateString("en-US", options) + " -04:00";
    signin.lastLoginFooter().should("contain", "Last login: " + dateTimeStr);
  ...
  it("TestCase3:Assertions on Name Sidebar", () => {
    ...
    dashboard
      ...
      .should(
        "contain.text",
        "ATAT uses Global Directory for CAC authentication."
      );
